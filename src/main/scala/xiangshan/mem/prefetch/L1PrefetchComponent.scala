...
trait HasL1PrefetchHelper extends HasCircularQueuePtrHelper with HasDCacheParameters with HasL1PrefetchSourceParameter {
  // region related
...
trait HasTrainFilterHelper extends HasCircularQueuePtrHelper {
  def reorder[T <: LsPipelineBundle](source: Vec[ValidIO[T]]): Vec[ValidIO[T]] = {
    if(source.length == 1) {
      source
...
class TrainFilter(size: Int, name: String)(implicit p: Parameters) extends XSModule with HasL1PrefetchHelper with HasTrainFilterHelper with HasPerfLogging {
...
class MutiLevelPrefetchFilter(implicit p: Parameters) extends XSModule with HasL1PrefetchHelper with HasPerfLogging {
...
class L1Prefetcher(implicit p: Parameters) extends BasePrefecher with HasStreamPrefetchHelper with HasStridePrefetchHelper {
  ...
  val stride_train = IO(Flipped(Vec(exuParameters.LduCnt, ValidIO(new LsPipelineBundle()))))
