...
class PrefetcherMonitor()(implicit p: Parameters) extends XSModule with HasPrefetcherMonitorHelper with HasStreamPrefetchHelper with HasPerfLogging {
  ...
  // depth_const := Constantin.createRecord(s"depth${p(XSCoreParamsKey).HartId}", initValue = 32.U)
  depth_const := 32.U
  ...
  // val enableDynamicPrefetcher_const = Constantin.createRecord(s"enableDynamicPrefetcher${p(XSCoreParamsKey).HartId}", initValue = 1.U)
  val enableDynamicPrefetcher_const = 1.U
