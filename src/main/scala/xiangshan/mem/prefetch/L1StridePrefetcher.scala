...
class StrideMetaArray(implicit p: Parameters) extends XSModule with HasStridePrefetchHelper with HasPerfLogging {
  ...
  // val always_update = Constantin.createRecord(s"always_update${p(XSCoreParamsKey).HartId}", initValue = ALWAYS_UPDATE_PRE_VADDR.B)
  ...
  when(s1_alloc) {
    ...
    val res = array(s1_index).update(s1_vaddr, always_update =/= 0.U)
    s1_can_send_pf := res._1
    s1_new_stride := res._2
  }

  // val l1_stride_ratio_const = Constantin.createRecord(s"l1_stride_ratio${p(XSCoreParamsKey).HartId}", initValue = 2.U)
  val l1_stride_ratio_const = 2.U
  val l1_stride_ratio = l1_stride_ratio_const(3, 0)
  // val l2_stride_ratio_const = Constantin.createRecord(s"l2_stride_ratio${p(XSCoreParamsKey).HartId}", initValue = 5.U)
  val l2_stride_ratio_const = 5.U
