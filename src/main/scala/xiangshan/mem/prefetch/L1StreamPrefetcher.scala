...
import xiangshan.mem.GatedValidRegNext
...
class StreamBitVectorArray(implicit p: Parameters) extends XSModule with HasStreamPrefetchHelper with HasPerfLogging {
  ...
  // val ratio_const = Constantin.createRecord(s"l2DepthRatio${p(XSCoreParamsKey).HartId}", initValue = L2_DEPTH_RATIO.U)
  val ratio_const = L2_DEPTH_RATIO.U
  val ratio = ratio_const(3, 0)

  // val l3_ratio_const = Constantin.createRecord(s"l3DepthRatio${p(XSCoreParamsKey).HartId}", initValue = L3_DEPTH_RATIO.U)
  val l3_ratio_const = L3_DEPTH_RATIO.U
  ...
  // val enable_l3_pf = Constantin.createRecord(s"enableL3StreamPrefetch${p(XSCoreParamsKey).HartId}", initValue = false.B)
  val enable_l3_pf = false.B
  // s5: send the l3 prefetch req out
  val s5_pf_l3_valid = GatedValidRegNext(s4_pf_l2_valid) && (enable_l3_pf =/= 0.U)
