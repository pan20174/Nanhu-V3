...
class ICacheMainPipe(implicit p: Parameters) extends ICacheModule with HasPerfLogging {
  ...
  (0 until PortNumber).map{ i =>
    ...
    io.errors(i).report_to_beu := io.csr_parity_enable && RegNext(s1_fire) && (s2_meta_errors(i) || s2_data_errors(i))
    io.errors(i).paddr         := s2_req_paddr(i)
    io.errors(i).source        := DontCare
    io.errors(i).source.tag    := s2_meta_errors(i)
    io.errors(i).source.data   := s2_data_errors(i)
  ...
  io.topdownIcacheMiss := !s2_fetch_finish
  io.topdownItlbMiss := s0_valid && !fromWayLookup.valid
