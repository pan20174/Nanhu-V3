...
class CompressUnit(implicit p: Parameters) extends XSModule{
  ...
  val noExc = io.in.map(!_.bits.cf.exceptionVec.asUInt.orR)
  val uopCanCompress = io.in.map(_.bits.ctrl.robCanCompress)
  val canCompress = io.in.zip(noExc).zip(uopCanCompress).map { case ((in, noExc), canComp) =>
    in.valid && !CommitType.isFused(in.bits.ctrl.commitType) && noExc && canComp
  } 
