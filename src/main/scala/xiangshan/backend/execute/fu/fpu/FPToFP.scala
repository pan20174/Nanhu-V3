...
class FPToFPDataModule(latency: Int)(implicit p: Parameters) extends FPUDataModule {
  ...
  val src1 = RegEnable(io.in.src(0), regEnables(0))
  val src2 = RegEnable(io.in.src(1), regEnables(0))
...
//  val lt = dcmp.io.lt || (dcmp.io.a.asSInt < 0.S && dcmp.io.b.asSInt >= 0.S)
  ...
  // new Module
  val fpToFp_new = Module(new FPToFP_new)
  fpToFp_new.io.src1 := src1
  fpToFp_new.io.src2 := src2
  fpToFp_new.io.tagIn := inTag
  fpToFp_new.io.tagOut := outTag
  fpToFp_new.io.wflag := wflags
  fpToFp_new.io.fcvt := ctrl.fcvt
  fpToFp_new.io.fpWen := ctrl.fpWen
  fpToFp_new.io.rm := rmReg

  io.out.data := RegEnable(fpToFp_new.io.data, regEnables(1))
  io.out.fflags := RegEnable(fpToFp_new.io.fflags, regEnables(1))
}
