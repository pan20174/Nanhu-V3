...
class FPToIntDataModule(latency: Int)(implicit p: Parameters) extends FPUDataModule {
  ...
  val src1_d = RegEnable(src1, regEnables(0))
  val src2_d = RegEnable(src2, regEnables(0))
  ...
  val op = Cat(ctrl_reg.typ(1), !ctrl_reg.typ(0))

  val fPToInt_new = Module(new FPToInt_new)
  fPToInt_new.io.src1 := src1_d
  fPToInt_new.io.src2 := src2_d
  fPToInt_new.io.tagIn := ctrl_reg.typeTagIn
  fPToInt_new.io.fmt := ctrl_reg.fmt
  fPToInt_new.io.op := op
  fPToInt_new.io.wflag := ctrl_reg.wflags
  fPToInt_new.io.fcvt := ctrl_reg.fcvt
  fPToInt_new.io.rm := rm_reg


  io.out.data := RegEnable(fPToInt_new.io.data, regEnables(1))
  io.out.fflags := RegEnable(fPToInt_new.io.fflags, regEnables(1))
}
