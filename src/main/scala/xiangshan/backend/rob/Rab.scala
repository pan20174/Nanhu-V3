...
class RenameBuffer(size: Int)(implicit p: Parameters) extends XSModule with HasCircularQueuePtrHelper with HasPerfLogging{
  ...
  for(i <- 0 until RabCommitWidth * RenameWidth) {
